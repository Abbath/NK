classdef TBlock
    ...
    methods
        ...
        function [obj1] = AlfaParams(obj)
            negF = 0;
            temp_F = 0;
            obj.alpha(3) = obj.MLayers(3).f;
            for lay = 4:(obj.NumItem-1)
                negF = obj.MLayers(lay-1).f;
                temp_F = obj.MLayers(lay).f;
                alfa(lay) = Compos(temp_F, negF);
            end
            negF = obj.MLayers(NumItem).f;
            obj.alfa(NumItem) = Compos(In_F, negF);
            obj1 = obj;
        end
        function [obj1] = Create_One_Layer(obj, int layer)
            obj1 = obj;
        ...
        function [obj1] = substructure(obj, direction, wav)
            ...
            if bottom_level <= obj.NumItem
                ...
                [f, alfa ] = LayerParameters(obj.NumItem, Ndx);
                if direction
                    [z1, t1] = Zstak(bottom_level, obj.r_o(wav), obj.r_o(wav), alfa, f, theta, obj.NumItem, pseud_nin, 1);
                    obj1.r_o(wav) = z1;
                    obj1.t_o(wav) = t1;
                else
                    [z1, t1] = Zstak(bottom_level, obj.r_o_back(wav), obj.r_o_back(wav), alfa, f, theta, obj.NumItem, pseud_nin, 1);
                    obj1.r_o(wav) = z1;
                    obj1.t_o(wav) = t1;
                end
            end
        ...
        function [obj1] = Back_Off_one(obj, layer)
            ...
            if layer > 1 && layer < obj.NumItem 
                ...
                for wav = 1:obj.numlam
                    ...
                    theta(2) = theta_fac*N;
                    [z1, t1] = Zstak(1, obj.r_o_back(wav), obj.t_o_back(wav),alfa, f, theta, 1, Nin, 1);
                    obj.r_o_back(wav) = z1;
                    obj.t_o_back(wav) = t1;
