classdef TBlock
    ...
    methods
        ...
        function [obj1] = AlfaParams(obj)
            ...
            obj.alfa(obj.NumItem+2) = Compos(obj.In_F, negF);
            obj1 = obj;
        end
        ...
        function [obj1] = substructure(obj, direction, wav)
            ...
            Maxlay_plus1 = obj.NumItem + 2;
            Ndx(Maxlay_plus1) = 1;
            Ndx(bottom_level - 1) = 1;
            theta(Maxlay_plus1) = 0;
            if bottom_level <= obj.NumItem + 1
                for lay = bottom_level:obj.NumItem + 1
                    level = l_bias + (2*direction - 1) * lay;
                    Ndx(level) = obj.n(lay);
                    if direction
                        obj = Create_One_Layer(obj, lay);
                    end
        ...
        function [obj2] = calc_superstructure(obj, wav)
            normal = 1;
            reverse = 0;
            obj1 = substructure(obj, normal, wav);
            obj2 = substructure(obj1, reverse, wav);
        end
        ...
        function [obj1] = BackOff_one(obj, layer)
            ...
            Ndx = zeros(1,3);
            theta = zeros(1,3);
            if layer > 2 && layer < obj.NumItem-1 
                ...
                for i = (layer+1):length(obj1.MLayers)-1;
                    obj1.MLayers(i) = obj.MLayers(i+1);
                end
            else
                obj1 = obj;
