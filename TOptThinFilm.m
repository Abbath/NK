classdef TOptThinFilm
    ...
    methods
        function f = TOptThinFilm()
            ...
            f.maxlay = data(1);
            counter = 3;
            for lay = 2:f.maxlay+1
                Re = data(4+((lay-2)*3));
                Im = data(5+((lay-2)*3));
                f.n(lay) = complex(Re, Im);
                f.d_orig(lay) = data(6+((lay-2)*3));
                f.n_opt(lay) = f.n(lay);
                counter = counter + 3;
            end
                ...
                for lay_col = 2:obj.maxlay+1
                    ...
                    for lay_row = 2:obj.maxlay+1
                        ...
                        if obj.iteration(obj.layer_to_vary) <= iter
                            ...
                            for thick_count = 2:obj.numthix_plus1
                                obj.Merit = 0.0;
                                ...
                                if obj.Merit < obj.BestMerit
                                    ...
                                    for i = 1:length(obj.head)
                                        for j = 2:obj.numlam+1
                                            tmp = obj.head(i);
                                            tmp.next_r_u(j) = tmp.r_comb(j);
                        ...
                        for i = 1:length(obj.head)
                            obj.head(i) = Adjust(obj.head(i), lay_row, obj.layer_to_vary, obj.r_u_updated, obj.d_sav);
                        end
        ...
        function [obj1, r_u_test] = recall(obj,n)
        ...
        function [Merit] = TotalMerit(obj, dx, layer_to_vary)
            TMerit = 0;
            for i = 1:length(obj.head)
                [~, tmp] = GetMerit(obj.head(i), dx, layer_to_vary);
                TMerit = TMerit + tmp;
            end
            if ~isempty(obj.head)
                Merit = TMerit / length(obj.head);
            else
                Merit = 0;
            end
        end
